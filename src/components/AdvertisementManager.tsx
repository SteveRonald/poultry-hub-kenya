    ...
    try {
      ...
      const response = await fetch(getApiUrl('/api/advertisements/reactivate'), {
        ...
        body: JSON.stringify({
          ...
          page_locations: null // Admin will set this during approval
        })
      });

      const data = await response.json();
      if (response.ok) {
        toast.success(data.message || 'Advertisement reactivated successfully. It will be reviewed by admin.');
                  ...
                  {isAdExpired(ad) && (
                    <>
                      <Button
                        variant="outline"
                        size="sm"
                        className="text-green-600 hover:text-green-700 flex items-center gap-1 sm:gap-2"
      ...
      <Dialog open={!!reactivatingAd} onOpenChange={(open) => !open && setReactivatingAd(null)}>
        <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Edit your advertisement details and reactivate it. The new price will be calculated based on tier and duration.
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            {/* Tier Selection */}
            ...
            {/* Note about page selection */}
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
              <p className="text-sm text-yellow-800">
                <strong>Note:</strong> The reactivated ad will be submited to Admin for approval.
              </p>
            </div>
          ...
          <DialogFooter>
            ...
            <Button 
              ...
              disabled={reactivating || uploadingImage}
              className="bg-green-600 hover:bg-green-700"
            >
              {reactivating ? 'Reactivating...' : 'Reactivate Advertisement'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
