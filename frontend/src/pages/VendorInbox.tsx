  ...
  return (
    <div>
      <div className="py-4 px-3 sm:px-4">
        <div>
          <h1 className="text-lg sm:text-xl font-semibold mb-4">Customer Messages</h1>
          {loading && <div>Loading...</div>}
          {!loading && conversations.length === 0 && <div>No conversations found.</div>}
          <div className="space-y-3">
            {conversations.map((conv, idx) => (
              <div key={idx} className="p-3 sm:p-4 bg-white rounded shadow flex justify-between items-start gap-3">
                <div className="flex gap-3 flex-1 min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-sm sm:text-base">{conv.product_name || 'Product'}</div>
                    <div className="text-xs sm:text-sm text-gray-600 mb-1">From: {conv.customer_name || `Customer`}</div>
                    <div className="text-xs text-gray-500 line-clamp-2">{conv.last_message || 'No message'}</div>
                    {conv.unread_count > 0 && (
